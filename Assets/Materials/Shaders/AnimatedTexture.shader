Shader "Animated/AnimatedTexture"
{
	...
	// albedo stuff
		...
		_Color2("Tex Color 2",Color) = (1,1,1,1)		
		// maintex
		...
		// emission brightness
		_EmissionBrightness("Emission Brightness", Range(0,10)) = 1
		// height map direcition, 
	...
	//	float _Cutoff
		...
		//				.75
			...
			float4 color = lerp(_Color, _Color2, tex2D(_ParallaxMap, IN.uv_ParallaxMap + _Time.y * _ParallaxDirection * _ParallaxSpeed * 0.1));
			// c = the maintexture and uv 
            fixed4 c = tex2D(_MainTex, IN.uv_MainTex + direction * _Time.x * _Speed ) * color;
			// albedo rgb
            o.Albedo = c.rgb;
			// apply metallic
			...
			//apply emission
			o.Emission = tex2D(_EmissionMap, IN.uv_MainTex).rgb * color * _EmissionBrightness;
			// apply alpha
            o.Alpha = c.a;
